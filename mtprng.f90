...
contains
  ...
  subroutine mtprng_init(seed, state)
    ...
    do i = 1, N - 1
      state%mt(i) = iand(4294967295_INT64,1812433253_INT64 * &
        ieor(state%mt(i-1),ishft(state%mt(i-1),-30_INT64)) + i)
    end do
    !
    state%mti = N
    !
  ...
  subroutine mtprng_init_by_array(init_key, state)
    ...
    do k = max(N,key_length), 0, -1
      !
      state%mt(i) = ieor( state%mt(i), &
        ieor( state%mt(i-1), ishft(state%mt(i-1), -30_INT64) * 1664525_INT64 ) &
        ) + init_key(j) + j
      !
      i = i + 1
      j = j + 1
    ...
    do k = N-1, 0, -1
      !
      state%mt(i) = ieor( state%mt(i), &
        ieor( state%mt(i-1), ishft(state%mt(i-1),-30_INT64) * 1566083941_INT64 ) &
        ) - i
      !
      i = i + 1
      if (i>=N) then
  ...
  if (state%mti >= N) then
    ...
    if (state%mti == -1) call mtprng_init(4357_INT32,state)
      ...
      do kk = 0, N - M - 1
        ...
        state%mt(kk) = ieor( ieor( state%mt(kk + M), ishft(r,-1_INT64) ), &
                             mag01( iand(r,1_INT64) ) )
      end do
      !
      do kk = N - M, N - 2
        r = ior( iand( state%mt(kk  ), UPPER_MASK ), &
                 iand( state%mt(kk+1), LOWER_MASK ) )
        state%mt(kk) = ieor( ieor( state%mt(kk+(M-N)), ishft(r,-1_INT64) ), &
                             mag01(iand(r,1_INT64)) )
      end do
      !
      r = ior( iand( state%mt(N-1), UPPER_MASK ), &
               iand( state%mt(0  ), LOWER_MASK ) )
      state%mt(N-1) = ieor( ieor( state%mt(M-1), ishft(r,-1) ), &
                            mag01(iand(r,1_INT64)) )
